...
class TGAccountsPreferencesViewController: TGPreferencesViewController, NSTableViewDelegate, NSTableViewDataSource {
    ...
    private func setupUI() {
        ...
        let signal = context.sharedContext.activeAccountsWithInfo |> mapToSignal { primary, accounts -> Signal<([AccountWithInfo], [PeerId : (PeerView, CGImage?)]), NoError> in
            let complementary = accounts.map { info -> Signal<(PeerId, (peerView: PeerView, photo: CGImage?)), NoError> in
                return combineLatest(info.account.viewTracker.peerView(info.account.peerId), peerAvatarImage(account: info.account, photo: .peer(info.peer, info.peer.smallProfileImage, info.peer.displayLetters, nil), displayDimensions: NSMakeSize(48, 48), scale: System.backingScale))
                    |> map { peerView, photo in
                        (info.account.peerId, (peerView: peerView, photo: photo.0))
                    }
            }
